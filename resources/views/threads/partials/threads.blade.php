            ...
            <li class="list-group-item media {!! !$column ?'':'col-sm-6' !!}" style="margin-top: 0px;">
                ...
                <div class="avatar pull-left">
                    <a href="{{ $thread->author_url }}">
                        <img class="media-object img-thumbnail avatar-48" alt="{{{ $thread->user->username }}}"
                             src="{{ $thread->user->avatar_small}}"/>
                    </a>
                </div>
                ...
                <div class="infos">
                    ...
                    <div class="media-body meta">
                        @if ($thread->like_count > 0)
                            <a href="{{ route('thread.show', [$thread->id]) }}" class="remove-padding-left"
                               id="pin-{{ $thread->id }}">
                                <span class="fa fa-thumbs-o-up"> {{ $thread->like_count }} </span>
                            </a>
                            <span> • </span>
                        @endif
                        <a href="{{ $thread->node->url }}"
                           title="{{{ $thread->node->name }}}" {{ $thread->like_count == 0 || 'class="remove-padding-left"'}}>
                            {{{ $thread->node->name }}}
                        </a>
                        @if ($thread->reply_count == 0)
                            <span> • </span>
                            <a href="{{ $thread->author_url }}"
                               title="{{{ $thread->user->username }}}">{{{ $thread->user->username }}}
                            </a>
                            <span> • </span>
                            <span class="timeago" data-toggle="tooltip" data-placement="top"
                                  title="{{ $thread->created_at }}">{{ $thread->created_at }}</span>
                        @endif
