    ...
    <div class="col-md-9 threads-show main-col">
        ...
        <div class="thread panel panel-default">
            <div class="infos panel-heading">
                ...
                <div class="likes">
                    <a href="javascript:void(0);" data-action="like" data-type="Thread"
                       data-url="{{ route('like.store') }}" title="{{ trans('hifone.like') }}"
                       class="fa fa-chevron-up likeable like" data-id="{{ $thread->id }}"> {{ $thread->like_count }}</a>
                    <a href="javascript:void(0);" data-action="unlike" data-type="Thread"
                       data-url="{{ route('like.destroy', $thread->id) }}" title="{{ trans('hifone.unlike') }}"
                       class="fa fa-chevron-down likeable like" data-id="{{ $thread->id }}"></a>
                </div>
            ...
            @foreach ($thread->appends as $index => $append)
                ...
                <div class="appends">
                    <span class="meta">{{ trans('hifone.appends.appends') }} {{ $index + 1 }} &nbsp;·&nbsp; <abbr
              ...
              <small>{!! trans('hifone.photos.drag_drop') !!}</small>
                    ...
                    @else
                        <div style="padding:20px;">
                            需要 <a class="btn btn-success" href="/auth/login">登录</a> 后方可回复, 如果你还没有账号请点击这里 <a
                                    class="btn btn-primary" href="/auth/register">registered</a>。
                        </div>
                    @endif
                </div>
            </div>
