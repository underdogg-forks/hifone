            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body ">
                    <ul class="nav nav-tabs" role="tablist">
                        <li @if(!$tab) class="active" @endif><a
                                    href="{{ route('user.edit',['id'=>$user->id]) }}">个人信息</a></li>
                        <li @if($tab=='avatar') class="active" @endif><a
                                    href="{{ route('user.edit',['id'=>$user->id,'tab'=>'avatar']) }}">头像更新</a></li>
                        <li @if($tab=='password') class="active" @endif><a
                                    href="{{ route('user.edit',['id'=>$user->id,'tab'=>'password']) }}">密码设置</a></li>
                    </ul>
                    @if($tab == 'avatar')
                        <form class="form-horizontal" method="post" action="/settings/update-avatar"
                              enctype="multipart/form-data" id="avatar-form">
                            {{ csrf_field() }}
                            <h5><i class="fa fa-image"></i>&nbsp;&nbsp;头像设置</h5>
                            <hr>
                            <div class="form-group">
                                <div class="col-sm-4 avatar-setting-container">
                                    <img src="{{Auth::user()->avatar}}" id="avatar" class="upload-btn"
                    ...
                    <span class="help-block">
                        头像支持jpg和png格式，上传的文件大小不超过 2M</span>
                                    <button type="submit" class="btn btn-primary hidden" id="avatarinput-submit">更新
                                    </button>
                                </div>
                            </div>
                        </form>
                    @elseif($tab == 'password')
                        <form class="form-horizontal" method="post" action="/settings/resetPassword"
                              enctype="multipart/form-data">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <h5><i class="fa fa-wrench"></i>&nbsp;&nbsp;密码设置</h5>
                            <hr>
                            <div class="form-group">
                                <div class="col-sm-4">
                                    <input type="password" name="old_password" placeholder="请输入您当前的密码"
                                           class="form-control">
                                </div>
                            </div>
                            <div class="form-group">
