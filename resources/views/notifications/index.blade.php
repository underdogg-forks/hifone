...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-heading clearfix">
          ...
          <a class="btn btn-sm btn-danger" rel="nofollow" data-method="post"
             data-url="/notification/clean">{{ trans('forms.clean') }}</a>
      </span>
        </div>
                                ...
                                <div class="avatar pull-left">
                                    <a href="{!! route('user.show', [$notification->from_user_id]) !!}">
                                        <img class="media-object img-thumbnail avatar"
                                             ...
                                             src="{!! $notification->fromUser->avatar_small !!}"
                                ...
                                <div class="infos">
                                    ...
                                    <div class="media-heading">
                                        ...
                                        @if($notification->type != 'user_follow')
                                            <a href="{!! route('thread.show', [$notification->thread->id]) !!}{!! !empty($notification->reply_id) ? '#reply' . $notification->reply_id : '' !!}"
                                               title="{!! $notification->thread->title !!}">
                                                {!! str_limit($notification->thread->title, '100') !!}
                                            </a>
                                        @endif
                                        <span class="meta">
                                 • {!! trans('hifone.at') !!} • <span
                                                    class="timeago">{!! $notification->created_at !!}</span>
                            </span>
                                    </div>
