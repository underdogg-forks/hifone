                ...
                @forelse($nodes as $node)
                    <div class="row striped-list-item" data-item-id="{{ $node->id }}">
                        ...
                        <div class="col-xs-5 text-right">
                            <a href="{{ route('dashboard.node.edit',['id'=>$node->id]) }}"
                               class="btn btn-default btn-sm">{{ trans('forms.edit') }}</a>
                            <a data-url="{{ route('dashboard.node.destroy',['id'=>$node->id]) }}"
                               class="btn btn-danger btn-sm confirm-action"
                               data-method="delete">{{ trans('forms.delete') }}</a>
                        </div>
                    </div>
                @empty
                    <div class="list-group-item"><a
                                href="{{ route('dashboard.node.create') }}">{{ trans('dashboard.nodes.add.message') }}</a>
                    </div>
                @endforelse
            </div>
        </div>
