            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <div class="col-sm-4">
                        ...
                        <p>{{ trans('dashboard.settings.aboutus.webserver') }}<span
                                    class="pull-right">{{ Request::server('SERVER_SOFTWARE') }}</span></p>
                        <p>{{ trans('dashboard.settings.aboutus.db') }}<span
                                    class="pull-right">{{ Config::get('database.default') }}</span></p>
                        <p>{{ trans('dashboard.settings.aboutus.cache') }}<span
                                    class="pull-right">{{ Config::get('cache.default') }}</span></p>
                        <p>{{ trans('dashboard.settings.aboutus.session') }}<span
                                    class="pull-right">{{ Config::get('session.driver') }}</span></p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="panel-body">
                    <div class="col-sm-4">
                        ...
                        @foreach($recent_threads as $index => $thread)
                            <p> {{ $index+1 }}. <a href="{{ $thread->url }}" target="_blank"> {{ $thread->title }}</a>
                            </p>
                        @endforeach
                    </div>
                    <div class="col-sm-6">
                        ...
                        @foreach($recent_replies as $index => $reply)
                            <p> {{ $index+1 }}. <a href="{{ $reply->url }}" target="_blank">
