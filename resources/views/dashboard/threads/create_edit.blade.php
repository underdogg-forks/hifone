            ...
            <div class="col-md-12">
                ...
                <fieldset>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="thread[title]" id="thread-title" required
                               value="{{ isset($thread) ? $thread->title : null }}">
                    </div>
                    <div class="form-group">
                            ...
                            @foreach ($sections as $section)
                                <optgroup label="{{{ $section->name }}}">
                                    @foreach ($section->nodes as $snode)
                                        <option value="{{ $snode->id }}" {!! (Input::old('node_id') == $snode->id || (isset($node) && $node->id==$snode->id)) ? 'selected' : ''; !!} >
                                            - {{ $snode->name }}</option>
                                    @endforeach
                                </optgroup>
                            @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <div class='markdown-control'>
                            <textarea name="thread[body]" class="form-control"
                                      rows="10">{{ isset($thread) ? $thread->body_original : null }}</textarea>
                        </div>
                    </div>
                </fieldset>
                ...
                <div class='form-group'>
                    <div class='btn-group'>
                        ...
                        <a class="btn btn-default"
                           href="{{ route('dashboard.thread.index') }}">{{ trans('forms.cancel') }}</a>
                    </div>
                </div>
                </form>
            </div>
